...
EOD;
    ...
    $stmt = $this->conn->prepare('select id, nom from auteurs');
    $stmt->execute();
    return $stmt->fetchAll();
}
...
EOD;
    $stmt = $this->conn->prepare($sql);
		$stmt->binvdValue('authorid',$authorID);
    $stmt->execute();
    return $stmt->fetchAll();
	}
	...
	function getBooksByYear(int $year) : array {
		$sql = <<<EOD
			select titre, annee, categorie, serie, couverture, auteurs
			from livres_complets
			where id in (select livre from ecriture where year=:yearid)
EOD;
    $stmt = $this->conn->prepare($sql);
		$stmt->binvdValue('yearid',$year);
    $stmt->execute();
    return $stmt->fetchAll();
	}
